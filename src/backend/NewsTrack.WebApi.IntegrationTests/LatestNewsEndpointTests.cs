...
{
    public class LatestNewsEndpointTests : BaseTest
    {
        private const string Endpoint = "/api/news/latest";

        public LatestNewsEndpointTests(TestWebAppFactory<Startup> testWebAppFactory)
            : base(testWebAppFactory)
        {
        }

        [Fact]
        public async Task GivenExistingNews_WhenGettingLatest_ThenReturnsCollection()
        {
            // Arrange
            const uint page = 0;
            const uint count = 10;
